...
func init() {
	// Wire the seam used by createGenericUprobeSensor (which lives in a
	// build-tag-agnostic file and cannot reference pod-informer code directly).
	resolvePathInContainerHook = registerResolvePathInContainer
}
...
func RegisterResolvePathInContainerPodHandlers(src events.PodEventSource) error {
	h := newUprobePodHandlers(ricRegistry, func(pod *v1.Pod) []string {
		return ricRegistry.matchingPolicies(pod.Namespace, pod.Labels)
	})
	return h.register(src)
}
...
// selectorMatcher builds a podMatcher from a policy's pod selector, using the
// same selector machinery as policyfilter (labels.SelectorFromLabelSelector).
// A nil selector matches all pods. The selector is compiled once; matching
// builds a label set from the pod's labels (plus the namespace label) per call.
func selectorMatcher(sel *slimv1.LabelSelector) podMatcher {
	if sel == nil {
		return nil
	}
	selector, err := labels.SelectorFromLabelSelector(sel)
	if err != nil {
		logger.GetLogger().Warn("uprobe resolvePathInContainer: invalid podSelector, matching no pods", "error", err)
...
func registerResolvePathInContainer(spec *v1alpha1.TracingPolicySpec, polInfo *policyInfo) (postLoad, postUnload sensors.SensorHook) {
	...
	match := selectorMatcher(spec.PodSelector)
	...
	postLoad = func() error {
		...
		ricRegistry.registerWithMatcher(key, rec, match)
		ricResync.ref()
...
func snapshotExistingContainers(key string, rec *containerUprobeReconciler, match podMatcher) {
	...
	matched := 0
	for _, c := range all {
		if match != nil && !match(c.Namespace, c.PodLabels) {
			continue
		}
		matched++
		rec.onContainerAdd(matchedContainer{
			key: containerKey(c.PodUID, c.ID),
		})
	}
...
// containers are skipped; containers previously skipped because their root was
// not resolvable yet attach once the runtime hook / CRI can resolve them.
//
// Cost is bounded: each not-yet-attached match does at most one CRI
// ContainerStatus call (capped by the attach cap and the per-call timeout), and
// missed ticks coalesce, so a slow resync only delays the next one.
func resyncAllPolicies() {
	for _, c := range discoverExistingContainers() {
		k := containerKey(c.PodUID, c.ID)
		for _, pol := range ricRegistry.matchingPolicies(c.Namespace, c.PodLabels) {
			ricRegistry.containerAdded(pol, k)
		}
	}
}
