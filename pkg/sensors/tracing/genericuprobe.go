...
func loadSingleUprobeSensor(uprobeEntry *genericUprobe, args sensors.LoadProbeArgs) error {
	...
	attachData := &program.UprobeAttachData{
		...
		LinkByFD:     uprobeEntry.loadArgs.linkByFD,
	}
...
func loadMultiUprobeSensor(ids []idtable.EntryID, args sensors.LoadProbeArgs) error {
	...
	for index, id := range ids {
		...
		attach.LinkByFD = attach.LinkByFD || uprobeEntry.loadArgs.linkByFD
...
func createGenericUprobeSensor(
	...
	for _, uprobe := range spec.UProbes {
		...
		entryFile, ok := openedFiles[absPath]
		if !ok {
		...
		ids, dna, err = addUprobe(&uprobe, entryFile, ids, &in, &has)
		if err != nil {
...
func addUprobeEntries(spec *v1alpha1.UProbeSpec, ids []idtable.EntryID, state *uprobeConfigState) ([]idtable.EntryID, error) {
	addUprobeEntry := func(sym string, offset uint64, idx int) error {
		...
		state.disableNotAllowed = state.disableNotAllowed || uprobeEntry.loadArgs.linkByFD
...
func addUprobe(spec *v1alpha1.UProbeSpec, entryFile *os.File, ids []idtable.EntryID, in *addUprobeIn, has *uprobeHas) (retIDs []idtable.EntryID, disableNotAllowed bool, retErr error) {
	...
	ids, err := addUprobeEntries(spec, ids, &state)
	if err != nil {
		return ids, false, err
	}

	return ids, state.disableNotAllowed, nil
}
