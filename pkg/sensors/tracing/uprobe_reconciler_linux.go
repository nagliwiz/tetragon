...
// matchedContainer is a container selected by the policy that the reconciler
// should attach to. key uniquely identifies it (e.g. "<podID>/<containerID>");
// the container id embedded in the key drives root resolution via the runtime
// hook / CRI (see rootResolver).
...
// containerRoot returns a directory the agent can open as c's container root,
// via the injected resolver. Returns "" when no root can be determined yet
// (resolver unset, or neither the runtime hook nor CRI resolves c).
func (r *containerUprobeReconciler) containerRoot(c matchedContainer) string {
	if r.resolveRoot == nil {
		return ""
	}
	return r.resolveRoot(containerIDFromKey(c.key), r.procFS)
}
...
// onContainerAdd resolves every RIC uprobe's in-container path for c and
// attaches them. Resolution is all-or-nothing per container: if any path (or
// BTF path) fails, the whole container is skipped and retried later. Failures
// are isolated to this container (other containers are unaffected).
func (r *containerUprobeReconciler) onContainerAdd(c matchedContainer) {
	...
	if _, already := r.attached[c.key]; already {
		r.mu.Unlock()
		return
	}
	r.wanted[c.key] = struct{}{}
	r.mu.Unlock()

	root := r.containerRoot(c)
	if root == "" {
	...
	resolve := func(p string) (string, bool) {
		...
		if err != nil {
			logger.GetLogger().Warn("uprobe reconciler: failed to resolve container path, skipping container",
				logfields.Error, err, "key", c.key, "root", root, "path", p)
			return "", false
		}
	...
	if _, active := r.wanted[c.key]; !active {
	...
	if len(r.attached) >= r.maxContainers {
		...
		logger.GetLogger().Debug("uprobe reconciler: per-policy container cap reached, skipping container",
			"key", c.key, "cap", r.maxContainers)
		return
	}

	if err := r.att.Attach(c.key, resolved); err != nil {
		logger.GetLogger().Warn("uprobe reconciler: failed to attach uprobe in container, skipping container",
			logfields.Error, err, "key", c.key)
		return
	}
	r.attached[c.key] = struct{}{}
}

// snapshot attaches all currently-matching containers. It is used when a
// resolvePathInContainer policy is registered while matching pods already exist,
// and on the periodic resync to re-attach via CRI discovery. It is idempotent:
// containers already attached are skipped by onContainerAdd.
func (r *containerUprobeReconciler) snapshot(containers []matchedContainer) {
