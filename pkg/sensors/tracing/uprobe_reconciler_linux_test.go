...
func TestReconcilerAttachContainer(t *testing.T) {
	...
	r.onContainerAdd(matchedContainer{key: "podA/c1"})
...
func TestReconcilerSoftFailPerContainer(t *testing.T) {
	...
	r.onContainerAdd(matchedContainer{key: "podA/c1"})
	r.onContainerAdd(matchedContainer{key: "podB/c2"}) // fails
	r.onContainerAdd(matchedContainer{key: "podC/c3"})
...
// T6: snapshot attaches all currently-matching containers at once (policy
// applied after pods already exist / agent restart re-snapshot). It is
// idempotent with respect to already-attached containers.
func TestReconcilerSnapshot(t *testing.T) {
	att := newFakeAttacher()
	r := newTestReconciler(att)

	// one container already attached before the snapshot.
	r.onContainerAdd(matchedContainer{key: "pod/c1"})

	r.snapshot([]matchedContainer{
		{key: "pod/c1"}, // already attached -> idempotent
		{key: "pod/c2"},
		{key: "pod/c3"},
	})
...
func TestReconcilerDetachAndIdempotentAdd(t *testing.T) {
	...
	r.onContainerAdd(matchedContainer{key: "podA/c1"})
	r.onContainerAdd(matchedContainer{key: "podA/c1"}) // duplicate
...
func TestReconcilerContainerCap(t *testing.T) {
	...
	r.onContainerAdd(matchedContainer{key: "pod/c1"})
	r.onContainerAdd(matchedContainer{key: "pod/c2"})
	r.onContainerAdd(matchedContainer{key: "pod/c3"}) // over the cap
	...
	r.onContainerAdd(matchedContainer{key: "pod/c3"})
...
func TestReconcilerConcurrentAddDelDetachAll(t *testing.T) {
	...
	// a concurrent snapshot, racing the adders/deleters.
	wg.Go(func() {
		batch := make([]matchedContainer, keys)
		for i := range keys {
			batch[i] = matchedContainer{key: keyName(i)}
		}
		for range 50 {
			r.snapshot(batch)
		}
	...
	r.onContainerAdd(matchedContainer{key: keyName(0)})
